...
export interface ModelConfig {
    id: string;
    name: string;
    provider: 'openai' | 'anthropic' | 'google' | 'xai' | 'ollama' | 'lmstudio' | 'azure' | 'custom';
    modelName: string;
    apiKey: string;
    baseUrl?: string;
    temperature: number;
...
export class LLMManager {
    ...
    private initializeClient(model: ModelConfig) {
        switch (model.provider) {
            case 'anthropic':
                this.clients.set(model.id, new Anthropic({
                    apiKey: model.apiKey
                }));
                break;
            case 'openai':
            case 'lmstudio':
            case 'custom':
                this.clients.set(model.id, new OpenAI({
    ...
    async getDefaultModelForPhase(phase: 'requirements' | 'design' | 'execution' | 'hooks'): Promise<string> {
        const config = vscode.workspace.getConfiguration('specCode');
        ...
        if (!firstModel) {
            throw new Error('No AI models configured. Please add a model in settings.');
        }
        return firstModel;
    }
    ...
    private async generateAnthropic(client: Anthropic, model: ModelConfig, messages: Message[]): Promise<LLMResponse> {
    ...
    private async generateOpenAI(client: OpenAI, model: ModelConfig, messages: Message[]): Promise<LLMResponse> {
    ...
    private async generateGoogle(client: GoogleGenerativeAI, model: ModelConfig, messages: Message[]): Promise<LLMResponse> {
    ...
    async generateWithTools(modelId: string, messages: Message[], tools: any[]): Promise<LLMResponse> {
        ...
        if (['openai', 'ollama', 'lmstudio', 'custom', 'xai', 'azure'].includes(model.provider)) {
            const response = await client.chat.completions.create({
                model: model.modelName,
                messages: messages.map(m => ({
                    role: m.role,
        ...
        if (model.provider === 'anthropic') {
            ...
            return {
                content: response.content[0]?.type === 'text' ? response.content[0].text : '',
                toolCalls: response.content.filter((c: any) => c.type === 'tool_use').map((c: any) => ({
                    id: c.id,
                    function: {
                        name: c.name,
        ...
        throw new Error(`Tool calling not supported for provider: ${model.provider}`);
    }

    async testModel(modelId: string): Promise<TestResult> {
        try {
            const response = await this.generate(modelId, [
                { role: 'user', content: 'Say "Spec-Code connection successful!" and nothing else.' }
            ]);
